...
export class WindowManager {
	...
	document.addEventListener("resize", this.changeTaskbarStaticTop.bind(this))
    }
    ...
    changeTaskbarStaticTop(e) {
	document.removeEventListener("mousemove", this.checkTaskbarChange.bind(this))
	this.taskbarStaticTop = this.taskbar.offsetTop
	document.addEventListener("mousemove", this.checkTaskbarChange.bind(this))
    }
}
...
export class Window {
	...
	document.addEventListener("mousemove", function(e) {
            if (windowDraggable && e.clientX > 0 && e.clientY > 0 && e.clientX < window.screen.width && e.clientY < targetWindow.wm.taskbarStaticTop) {
	...
	document.addEventListener("mousemove", function(e) {
		...
		updateCursor(getResizeStyleString(windowResizeX, windowResizeY))
            ...
            else if (e.clientX >= 0 && e.clientX < window.screen.width && e.clientY >= 0 && e.clientY < window.screen.height){
		velocityX = lastPosX - e.clientX
		velocityY = lastPosY - e.clientY
		lastPosX = e.clientX
		lastPosY = e.clientY
		...
		let barrierX = (targetWindow.windowNode.offsetLeft + (windowResizeX === 1 ? minWidth + padR : targetWindow.windowNode.offsetWidth - minWidth - padL)) * windowResizeX
		let barrierY = (targetWindow.windowNode.offsetTop + (windowResizeY === 1 ? minHeight + padB : targetWindow.windowNode.offsetHeight - minHeight - padT)) * windowResizeY
		if (newWidth > minWidth && e.clientX * windowResizeX > barrierX) {
		    targetWindow.windowNode.style.width = newWidth + "px"
		    if (windowResizeX === -1) {
			targetWindow.windowNode.style.left = (targetWindow.windowNode.offsetLeft - velocityX) + "px"
		    }
		}
		if (newHeight > minHeight && e.clientY * windowResizeY > barrierY) {
